...
export class ListarClienteComponent implements OnInit {
  ...
  initData() {
    ...
    this.service.pegarClientes(this.currentPage, this.pageSize).then(data => {
  ...
  pageChange(pageIndex) {
    ...
    if (loadedRecordCount <= lastRequestedRecordIndex) {
      ...
      this.service.pegarClientes(this.currentPage, this.pageSize)
        .then(data => {
          if (this.source.count() > 0) {
            data.content.forEach(d => this.source.add(d));
            this.source.getAll()
  ...
  async apagarCliente(event) {
    await this.service.deletarCliente(event.data.id).then(
      response => {
        if (response.status === 204) {
          this.initData();
          this.showToast('top-right', 'success', 'Legal', 'Cliente apagado com sucesso');
        } else {
          this.showToast('top-right', 'danger', 'Ops!', 'Erro ao apagado cliente');
        }
      });
  }
